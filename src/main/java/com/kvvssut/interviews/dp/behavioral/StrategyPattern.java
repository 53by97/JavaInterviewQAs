package com.kvvssut.interviews.dp.behavioral;

/*
 * There are common situations when classes differ only in their behavior.
 * For this cases is a good idea to isolate the algorithms in separate
...
/*
 * The strategy design pattern splits the behavior (there are many behaviors) of
 * a class from the class itself. This has some advantages, but the main draw
 * back is that a client must understand how the Strategies differ. Since
 * clients get exposed to implementation issues the strategy design pattern
